...
def convert_can_trace(
    ...
    Args:
        ...
        database_files: CAN databases for decoding, in precedence order (a
            later file wins a conflicting message id). Empty writes raw
            frames only.
        output_file: Destination .trz file
        progress_callback: Optional callback(message_count) for progress updates
        emit_schemas_on_init: Pre-generate all schemas before processing
        prefix: Leading trace-source name; empty names the source after the
            input file instead
...
Examples:
  ...
  python -m zelos_extension_can.converter capture.asc base.dbc overlay.dbc

  # Raw frames only, no database
  python -m zelos_extension_can.converter capture.asc
...
Supported formats: .asc, .blf, .trc, .log, .csv, .mf4
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="*",
        help="CAN database files (.dbc, .arxml, .kcd, .sym), in precedence order",
    )
    parser.add_argument(
        "--prefix",
    ...
    for database_file in args.database_files:
        if not database_file.exists():
            logger.error(f"CAN database file not found: {database_file}")
            sys.exit(1)
    ...
    try:
        ...
        _convert_with_progress(
            args.input_file, args.database_files, output_file, args.verbose, args.prefix
        )
...
def _convert_with_progress(
    ...
    Args:
        input_file: Source CAN log file
        database_files: CAN databases for decoding, in precedence order
        output_file: Destination .trz file
        verbose: Enable verbose output
        prefix: Leading trace-source name; empty names it after the input file
    """
    ...
    logger.info(f"Converting {input_file} -> {output_file}")
    logger.info("Using databases: %s", ", ".join(str(p) for p in database_files) or "(none)")
    ...
    print(f"  Databases: {', '.join(str(p) for p in database_files) or '(none)'}")
