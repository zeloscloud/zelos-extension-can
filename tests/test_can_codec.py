...
class TestErrorHandling:
    ...
    def test_missing_dbc_suggests_a_close_name(self, tmp_path):
        (tmp_path / "blue_marlin_full.dbc").touch()
        config = {"interface": "virtual", "database_files": [str(tmp_path / "blue_marlin.dbc")]}
        with (
            pytest.raises(FileNotFoundError, match=r"did you mean blue_marlin_full\.dbc\?"),
            patch("zelos_sdk.TraceSource"),
        ):
            CanCodec(config)
